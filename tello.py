...
#==========내부 사용 함수========================================================================================
    ...
    def __init__(self, command_timeout=0.3):

        #1) 인스턴스 변수들 및 소켓 생성
        self.command_timeout = command_timeout # command_timeout (float): 명령을 기다리는 시간(초)

        self.socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # IPv4, UDP 통신 소켓 객체를 생성(command용)
        self.socket.bind(('', 8889)) #소켓 객체를 텔로와 바인딩(8889 포트)

        self.socket_video = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # IPv4, UDP 통신 소켓 객체를 생성(video stream용)
        self.socket_video.bind(('', 11111)) #소켓 객체를 텔로와 바인딩(11111 포트)
        ...
        #2) Tello에게서 응답을 받을 thread 실행
        self.receive_thread = threading.Thread(target=self._receive_thread)
        self.receive_thread.daemon = True #오직 이 스레드만 실행 중인 경우 프로그램을 종료
    ...
    def __del__(self):
        self.socket.close() #command용 소켓 닫기
        self.socket_video.close() #video stream용 소켓 닫기
        print("[tello] tello_run: 연결 종료")
